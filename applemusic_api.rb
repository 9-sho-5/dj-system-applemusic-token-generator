module MusicApi
    class AppleMusicApi
        ...
        @@music_user_token = nil
    
        def initialize(music_user_token)
            @@music_user_token = music_user_token
            @apple_music_api = Faraday.new(:url => API_ENDPOINT)
            @apple_music_api.headers['Authorization'] = "Bearer #{@@client_access_token}"
            @apple_music_api.headers['Content-Type'] = 'application/json'
            @apple_music_api.headers['Accept-Language'] = 'ja'
            @apple_music_api.headers['Music-User-Token'] = music_user_token
        end
        ...
        def get_playlists(limit)
            res = @apple_music_api.get "me/library/playlists?extend&limit=#{limit}"
            if res.status == 401
                regenerate_access_token
                res = @apple_music_api.get "me/library/playlists?extend&limit=#{limit}"
            end
        ...
        def create_playlist(name, description)
            data = {
                attributes: {
                    name: name,
                    description: description ? description + " - Generated by AppleMusic Token Generator" : "Generated by AppleMusic Token Generator",
                }
            }
